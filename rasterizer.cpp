...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    int b_top = std::ceil(std::max(v[0][1], std::max(v[1][1], v[2][1])));
    int b_bottom = std::floor(std::min(v[0][1], std::min(v[1][1], v[2][1])));
    int b_left = std::floor(std::min(v[0][0], std::min(v[1][0], v[2][0])));
    int b_right = std::ceil(std::max(v[0][0], std::max(v[1][0], v[2][0])));

    for (int j = b_bottom; j < b_top; j++)
    {
